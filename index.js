    ...
    processor.onBlock(function(num, block) {
            ...
            for (var o in state.stats.offsets) {
                ...
                if (sun - state.stats.offsets[o] == 1500) {
                    state.refund.push(['sign', [
                        ["vote", { "author": streamname, "permlink": `h${num-300}`, "voter": username, "weight": 10000 }]
                    ]])
                }
            }
        ...
        \n`
            ...
            \n`
                if (state.news.i.length > 0) {
                    body = body + state.news.i[0] + footer;
                    state.news.i.shift();
                } else {
                    body = body + footer
                }
            ...
            \n`
                if (state.news.t.length > 0) {
                    body = body + state.news.t[0] + footer;
                    state.news.t.shift();
                } else {
                    body = body + footer
                }
...
function whotopay() {
    ...
    for (d in state.kudos) {
        ...
        if (state.kudos[d] == 1) { // for sorting , unshift 1 assuming most will be 1
            ...
            if (!o.length) {
                o.unshift({ //if nothing to sort, unshift into array
                    account: d,
